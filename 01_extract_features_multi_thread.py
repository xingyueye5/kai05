...
class VideoFrameLoader:
        ...
        with ThreadPoolExecutor(max_workers=self.num_workers) as executor:
            while True:
                ...
                with self.lock:
                    # 计算需要预加载的视频
                    while (
                        self.next_load_idx < len(self.video_tasks)
                        and len(self.video_cache) + len(videos_to_load) < self.prefetch_videos
                    ):
                ...
                # 等待当前视频被消费后再继续加载
                with self.lock:
                    while len(self.video_cache) >= self.prefetch_videos and self.current_video_idx < len(
                        self.video_tasks
                    ):
    ...
    def get_batch(self) -> tuple[list[VideoTask], list[int], list[np.ndarray]]:
        ...
        while len(frames) < self.batch_size:
            ...
            with self.lock:
                while (
                    self.current_video_idx not in self.video_cache
                    and not self.loading_done
                    and self.current_video_idx < len(self.video_tasks)
                ):
                    self.condition.wait(timeout=0.1)
...
class FeatureAccumulator:
    ...
    def add_features(self, video_tasks: list[VideoTask], frame_indices: list[int], features: torch.Tensor):
        """添加一批特征"""
...
def extract_features_from_dataset(
    ...
    for video_file in video_files:
        ...
        video_tasks.append(VideoTask(video_path=video_file, feature_path=feature_path, camera_key=camera_key))
    ...
    # 预先统计总帧数
    total_frames, frame_counts = count_total_frames(video_tasks, frame_interval=frame_interval, num_workers=num_workers)
